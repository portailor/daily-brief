...
카카오는 아무에게나 메시지를 못 보내게 막아 두었다. 보내려면 두 가지가 모두 필요하다.
  1) 보내는 사람(동화님)의 토큰에 'friends' 동의항목이 들어 있을 것
  2) 받는 사람이 이 앱에 카카오 로그인을 한 번 해서 '연결된 사용자'가 되어 있을 것
     (그리고 서로 카카오톡 친구여야 친구 목록에 나타난다)
